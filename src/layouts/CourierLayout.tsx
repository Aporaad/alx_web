...
import {
  LayoutDashboard, Truck, FileText, Megaphone,
  MessageSquare, User, LogOut, Menu, X, Globe, Sun, Moon, Search
} from 'lucide-react';
...
const navItems: NavItem[] = [
  { to: '/portal/courier',               icon: <LayoutDashboard size={16} />, labelKey: 'dashboard' },
  { to: '/portal/courier/tasks',         icon: <Truck size={16} />,           labelKey: 'myTasks' },
  { to: '/portal/courier/ledger',        icon: <FileText size={16} />,        labelKey: 'myEarnings' },
  { to: '/portal/courier/announcements', icon: <Megaphone size={16} />,       labelKey: 'announcements' },
  { to: '/portal/courier/tickets',       icon: <MessageSquare size={16} />,   labelKey: 'supportTickets' },
  { to: '/portal/courier/profile',       icon: <User size={16} />,            labelKey: 'profile' },
];
        ...
        <div style={{ padding: '1.25rem 1rem', borderBottom: '1px solid var(--bg-border)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: '0.6rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>{tr('courier')}</div>
            </div>
          </div>
        </div>
        <nav style={{ flex: 1, padding: '0.75rem 0', overflowY: 'auto' }}>
          {navItems.map(item => (
            <NavLink key={item.to} to={item.to} end={item.to === '/portal/courier'}
      ...
      <div style={{ flex: 1, marginInlineStart: window.innerWidth >= 1024 ? 240 : 0, display: 'flex', flexDirection: 'column' }}>
        <header style={{ position: 'sticky', top: 0, zIndex: 30, background: 'rgba(5,5,5,0.95)', backdropFilter: 'blur(12px)', borderBottom: '1px solid var(--bg-border)', padding: '0.6rem 1.25rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          ...
          <button className="btn btn-ghost btn-sm" onClick={toggleLang} style={{ gap: '0.3rem' }}>
            <Globe size={14} /><span style={{ fontSize: '0.7rem', fontWeight: 700 }}>{lang === 'ar' ? 'EN' : 'ع'}</span>
          </button>
          <div style={{ width: 28, height: 28, background: 'rgba(16,185,129,0.15)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.7rem', fontWeight: 800, color: '#34d399' }}>
